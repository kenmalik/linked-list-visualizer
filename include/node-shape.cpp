...
template <typename T> NodeShape<T>::NodeShape() {}
...
NodeShape<T>::NodeShape(const sf::Vector2f &size, const sf::Font &font)
    ...
    data.setOrigin(
        data.getGlobalBounds().left + data.getGlobalBounds().width / 2,
        data.getGlobalBounds().top + data.getGlobalBounds().height / 2);
    data.setPosition(
...
template <typename T>
void NodeShape<T>::draw(sf::RenderTarget &window,
...
template <typename T> void NodeShape<T>::setData(const Data<T> &data) {
    this->data = data;
    this->data.setFillColor(sf::Color::Black);
    this->data.setOrigin(this->data.getGlobalBounds().left +
                             this->data.getGlobalBounds().width / 2,
